...
class _GuardianModeScreenState extends State<GuardianModeScreen> {
  ...
  void _showAddContactDialog(BuildContext context, {int? editIndex}) {
    final nameController = TextEditingController(text: editIndex != null ? _contacts[editIndex]['name'] : '');
    final phoneController = TextEditingController(text: editIndex != null ? _contacts[editIndex]['phone'] : '');
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF111519),
        title: Text(editIndex != null ? 'Edit Contact' : 'Add Contact', style: const TextStyle(color: Colors.white)),
        content: Column(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel', style: TextStyle(color: Color(0xFF8892A4)))),
          if (editIndex != null)
            TextButton(
              onPressed: () { setState(() => _contacts.removeAt(editIndex)); Navigator.pop(ctx); },
          ...
          ElevatedButton(
            ...
            onPressed: () {
              if (nameController.text.isNotEmpty && phoneController.text.isNotEmpty) {
                setState(() {
                  if (editIndex != null) {
                    _contacts[editIndex] = {'name': nameController.text, 'phone': phoneController.text};
                  } else {
                    _contacts.add({'name': nameController.text, 'phone': phoneController.text});
                  }
                });
                Navigator.pop(ctx);
              }
